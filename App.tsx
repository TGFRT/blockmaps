...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setMessages([{
      id: 'init-1',
      role: 'model',
      text: '¡Hola! Soy tu guía de mapas. ¿Qué estás buscando hoy? Puedo ayudarte a encontrar restaurantes, tiendas o direcciones.',
      timestamp: new Date()
    }]);
  ...
  const handleSend = async () => {
    ...
    try {
      // Pass the current history excluding the one we just added locally (to avoid dupe if we handled differently)
      // actually geminiService takes the array of messages.
      ...
      const errorMessage: Message = {
        id: (Date.now() + 1).toString(),
        role: 'model',
        text: "Lo siento, tuve problemas para conectar con Google Maps. Por favor intenta de nuevo.",
        timestamp: new Date()
      };
    ...
    <div className="flex flex-col h-full bg-slate-50 relative">
      ...
      <header className="flex-none bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shadow-sm z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800 tracking-tight">GeoGuide</h1>
        ...
        <div className="flex items-center gap-2 bg-slate-100 px-3 py-1.5 rounded-full">
          ...
          <span className="text-xs font-semibold text-slate-600">
            {locStatus === 'connected' ? 'GPS Active' : locStatus === 'connecting' ? 'Locating...' : 'No Location'}
          </span>
        </div>
      </header>
      ...
      <footer className="flex-none bg-white p-4 border-t border-slate-200">
        <div className="max-w-3xl mx-auto w-full relative">
          <input
            ...
            placeholder="Pregunta sobre lugares, rutas..."
            value={input}
